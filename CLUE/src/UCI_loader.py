...
def load_UCI(dset_name, splits=10, seed=0, separate_targets=True, save_dir='data/'):
    ...
    if dset_name == 'boston':
        ...
        if not os.path.isfile(save_dir+'Concrete_Data.xls'):
            urllib.request.urlretrieve("https://archive.ics.uci.edu/ml/machine-learning-databases/concrete/compressive/Concrete_Data.xls",
                               filename=save_dir+'Concrete_Data.xls')
        data = pd.read_excel(save_dir+ 'Concrete_Data.xls', header=0).values#, delimiter="\s+").values
        y_idx = [-1]

    elif dset_name == 'energy':
        if not os.path.isfile(save_dir+'ENB2012_data.xlsx'):
            urllib.request.urlretrieve("http://archive.ics.uci.edu/ml/machine-learning-databases/00242/ENB2012_data.xlsx",
                               filename=save_dir+'ENB2012_data.xlsx')
        data = pd.read_excel(save_dir+'ENB2012_data.xlsx', header=0).values#, delimiter="\s+").values
        y_idx = [-2, -1]

    elif dset_name == 'power':
        if not os.path.isfile(save_dir+'CCPP.zip'):
            urllib.request.urlretrieve("https://archive.ics.uci.edu/ml/machine-learning-databases/00294/CCPP.zip",
                               filename=save_dir+'CCPP.zip')
        zipped = zipfile.ZipFile(save_dir+"CCPP.zip")
        data = pd.read_excel(zipped.open('CCPP/Folds5x2_pp.xlsx'), header=0).values#, delimiter="\t").values
        y_idx = [-1]

    elif dset_name == 'wine':
        if not os.path.isfile(save_dir+'winequality-red.csv'):
            print("hey ho gonna use the retriever!!!!!")
        ...
        if not os.path.isfile(save_dir+'yacht_hydrodynamics.data'):
            urllib.request.urlretrieve("http://archive.ics.uci.edu/ml/machine-learning-databases/00243/yacht_hydrodynamics.data",
        ...
        if not os.path.isfile(save_dir+'dataset_2175_kin8nm.csv'):
            urllib.request.urlretrieve("https://www.openml.org/data/get_csv/3626/dataset_2175_kin8nm.csv",
        ...
        if not os.path.isfile(save_dir + 'UCI%20CBM%20Dataset.zip'):
            urllib.request.urlretrieve("http://archive.ics.uci.edu/ml/machine-learning-databases/00316/UCI%20CBM%20Dataset.zip",
        ...
        if not os.path.isfile(save_dir+'CASP.csv'):
            urllib.request.urlretrieve("https://archive.ics.uci.edu/ml/machine-learning-databases/00265/CASP.csv",
        ...
        data = pd.read_excel(save_dir + 'default of credit card clients.xls', header=[0, 1], index_col=0).values#,delimiter="\s+").values
        y_idx = [-1]  # OK
