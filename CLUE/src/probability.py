...
def diagonal_gauss_loglike(x, mu, sigma):
    ...
    det_sig_term = -tf.math.log(sigma)
    inner = (x - mu)/sigma
    dist_term = -(0.5)*(inner**2)
    log_px = tf.reduce_sum(cte_term + det_sig_term + dist_term, axis=1)
    return log_px
...
def get_rms(mu, y, y_means, y_stds):
    ...
    return tf.reduce_sum(tf.sqrt(((x_un - y_un)**2)) / y.shape[0])
...
def gmm_likelihood(x, mu_vec, sigma_vec):
    ...
    loglike_terms = tf.concat(loglike_terms, axis=0)

    out = tf.math.reduce_logsumexp(loglike_terms, axis=0) - weight_factor
    return out
...
def get_loglike(mu, sigma, y, y_means, y_stds, gmm=False):
    ...
    return tf.math.reduce_mean(ll, axis=0)  # mean over datapoints
